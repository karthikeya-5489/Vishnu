...
app.post('/api/savesemisters', async (req, res) => {
  try {
    ...
    const { Semister, username, Branch,selectedSubject, userResponse } = req.body;
    const user = await User.findOne({ username });
  ...
  // }
    ...
    const newSemisterOption = new SemisterOption({ user: user._id, Semister, Branch,selectedSubject, userResponse });
